...
public class ReservationRepository {
    ...
    public Optional<Reservation> findById(UUID id) {
        return jReservationRepository.findById(id)
                .map(ReservationRepository::toDomain);
    }
    ...
    public Reservation save (Reservation reservation) {
        JProjection jProjection =
                jProjectionRepository
                        ...
                        .orElseThrow(
                                () -> new IllegalStateException("Projecition " +  reservation.getProjection().getId() + " not found"));
        JUser jUser =
                jUserRepository
                        .findById(reservation.getUser().getId())
                        .orElseThrow(
                                () -> new IllegalStateException("User " +  reservation.getUser().getId() + " not found"));
        Set<JSeat> jSeats =
                Set.copyOf(
