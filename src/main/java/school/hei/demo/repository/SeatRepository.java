...
public class SeatRepository {
  ...
  public List<Seat> findByRoomId(UUID roomId) {
    return jSeatRepository.findByRoomId(roomId).stream().map(SeatRepository::toDomain).toList();
  }

  public List<Seat> findByIdIn(List<UUID> ids) {
    return jSeatRepository.findByIdIn(ids).stream().map(SeatRepository::toDomain).toList();
  }
  ...
  public Seat save(Seat seat) {
    ...
    JSeat toSave = new JSeat(seat.getId(), seat.getNumber(), jRoom);
    return toDomain(jSeatRepository.save(toSave));
  }

  public static Seat toDomain(JSeat entity) {
    return new Seat(entity.getId(), entity.getNumber(), RoomRepository.toDomain(entity.getRoom()));
  }
}
