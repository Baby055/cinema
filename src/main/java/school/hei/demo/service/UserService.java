...
public class UserService {
  ...
  private UserRole resolveRoleForCreation(
      UserRole requested, Optional<AuthenticatedUser> currentUser) {
    boolean requesterIsManager =
        currentUser.map(u -> u.getRole() == UserRole.MANAGER).orElse(false);
  ...
  private User update(SaveUser toSave, Optional<AuthenticatedUser> currentUser) {
    ...
    User existing =
        userRepository
            .findById(toSave.id())
            .orElseThrow(() -> new NotFoundException("User " + toSave.id() + " not found"));
